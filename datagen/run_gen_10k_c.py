...
MODEL="deepseek-v4-pro"; CONC=48
prompts=[json.loads(l) for l in open("/data/scripts/Toucan/datagen/prompts_10k_c_prepared.jsonl")]
def gen(r):
    m=r["metadata"]
    try:
...
with open("/data/scripts/Toucan/datagen/questions_10k_c.jsonl","w") as f, \
...
print(f"GEN10KC_DONE 成功 {ok}/{len(prompts)}",flush=True)
