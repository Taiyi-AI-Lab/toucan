...
MODEL="deepseek-v4-pro"; CONC=32; NVAR=3
...
seeds=[json.loads(l) for l in open(f"{HERE}/questions_batch2_rollout.jsonl")]
...
with open(f"{HERE}/questions_batch2_augmented.jsonl","w") as f, concurrent.futures.ThreadPoolExecutor(max_workers=CONC) as ex:
...
print(f"AUGB2_DONE {done} 调用 → {nvar} 个增广问题",flush=True)
