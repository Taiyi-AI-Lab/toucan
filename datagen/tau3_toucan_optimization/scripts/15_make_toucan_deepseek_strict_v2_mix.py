...
def main():
    out_dir = Path(os.environ.get("TOUCAN_MIX_OUT_DIR", str(DATA_DIR / "toucan_optimized_mix_deepseek_strict_v2")))
    ...
    hard_path = Path(os.environ.get("HARD_SFT", str(DATA_DIR / "deepseek_toucan_hard_subset_strict_v2_sft.jsonl")))
    if not hard_path.exists():
        raise SystemExit(f"Missing {hard_path}; run scripts/14_deepseek_strict_v2_review.py first")
    hard = list(read_jsonl(hard_path))
    generated_path = Path(os.environ.get("GENERATED_SFT", str(DATA_DIR / "generated_tau3_style_clean_sft.jsonl")))
    ...
    manifest = {
        ...
        "strict_v2_hard_subset": len(hard),
        "generated": len(generated),
        "dropped_generated": dropped_generated,
        "train_formula": "base_train + 2x strict_v2_hard_subset + 3x generated",
        "eval_formula": "base_eval + 5% strict_v2_hard_subset cap200 + 5% generated cap200",
