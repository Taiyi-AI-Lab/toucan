...
max_workers=${8:-8}
vllm_api_url=${9:-${VLLM_API_URL:-"http://localhost:8000/v1"}}
openrouter_url=${OPENROUTER_URL:-""}
user_simulation_engine=${USER_SIMULATION_ENGINE:-""}
user_simulation_openrouter_url=${USER_SIMULATION_OPENROUTER_URL:-""}
...
if [ -n "$user_simulation_engine" ]; then
    echo -e "${BLUE}User simulation engine: ${user_simulation_engine}${NC}"
fi
if [ -n "$user_simulation_openrouter_url" ]; then
    echo -e "${BLUE}User simulation API URL: ${user_simulation_openrouter_url}${NC}"
fi
if [ -n "$openrouter_url" ]; then
    echo -e "${BLUE}OpenAI-compatible agent API URL: ${openrouter_url}${NC}"
fi
echo -e "${BLUE}Max workers: ${max_workers}${NC}"
echo -e "${BLUE}vLLM/SGLang API URL: ${vllm_api_url}${NC}"
echo -e "${BLUE}========================================${NC}"
...
elif [[ "$model_path" == *"deepseek"* ]] || [[ "$model_path" == *"DeepSeek"* ]]; then
    agent="openai_agent"
    engine="openrouter_api"
    model_impl="chat"
    echo -e "${BLUE}[OpenAI Agent] Auto-detected OpenAI-compatible Chat Agent ${NC} for ${model_path}"
else
    agent="qwen_agent"
...
if [ "$engine" == "vllm_api" ]; then
    if [ "$start_vllm_service" != "true" ]; then
        ...
        echo -e "${YELLOW}[VLLM API] Assuming OpenAI-compatible server is already running at ${vllm_api_url}${NC}"
    else
        # ------------------------------------------------------------------------------------------------
...
user_sim_args=()
if [ -n "$user_simulation_engine" ]; then
    user_sim_args+=(--user_simulation_engine "$user_simulation_engine")
fi
if [ -n "$user_simulation_openrouter_url" ]; then
    user_sim_args+=(--user_simulation_openrouter_url "$user_simulation_openrouter_url")
fi
agent_api_args=()
if [ -n "$openrouter_url" ]; then
    agent_api_args+=(--openrouter_url "$openrouter_url")
fi
...
if [ "$agent" == "openai_agent" ]; then
    ...
    python ext_multi_turn_openai_agent.py \
        ...
        --engine ${engine} \
        --model_impl ${model_impl:-responses} \
        "${agent_api_args[@]}" \
