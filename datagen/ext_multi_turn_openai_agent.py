...
def get_args():
    ...
    parser.add_argument("--user_simulation_engine", type=str, default=None, choices=["vllm_api", "openrouter_api"],
                        help="Engine for generating follow-up user queries. Defaults to --engine.")
    parser.add_argument("--user_simulation_openrouter_url", type=str, default=None,
                        help="OpenAI-compatible base URL for user simulation when using openrouter_api.")
    parser.add_argument("--user_simulation_openrouter_api_key", type=str, default="",
                        help="API key for user simulation. If empty, completion_endpoint.py reads OPENROUTER_API_KEY.")
    ...
    parser.add_argument("--model_impl", type=str, default="responses", choices=["responses", "chat"],
                        help="OpenAI Agents model implementation. Use chat for deepseek/dimcode.")
    parser.add_argument("--reasoning_effort", type=str, default="high", help="Reasoning effort")
    parser.add_argument("--parallel_function_calls", type=str2bool, default=True, help="Parallel function calls")
...
# Helper Functions
################
def shorten_text(value, limit=500):
    if value is None:
        return ""
...
def tool_call_summaries(msg):
    ...
    if isinstance(function_call, dict):
        ...
        args = shorten_text(function_call.get("arguments"), 200)
        calls.append(f"{name}({args})" if args else name)
    ...
    for tc in msg.get("tool_calls") or []:
        ...
        args = shorten_text(fn.get("arguments") or tc.get("arguments"), 200)
        calls.append(f"{name}({args})" if args else name)
    return calls


def is_complete_seed(item):
    messages = item.get("messages")
    if not isinstance(messages, list) or not messages:
        return False, "missing_messages"
    if item.get("ok") is False:
        return False, "not_ok"
    if item.get("finish") not in (None, "stop"):
        return False, f"finish_{item.get('finish')}"
    if messages[-1].get("role") != "assistant":
        return False, "last_not_assistant"
    return True, "ok"
...
def condense_conversation(messages):
    ...
    for msg in messages:
        ...
        if role == 'system':
            ...
            condensed.append(f"User: {shorten_text(content, 1000)}")
        elif role == 'assistant':
            visible = shorten_text(content, 1000)
            if visible:
                condensed.append(f"Assistant: {visible}")
            calls = tool_call_summaries(msg)
            if calls:
                condensed.append(f"Assistant tool calls: {'; '.join(calls)}")
            if not visible and not calls:
                condensed.append("Assistant: [empty response]")
        elif role in ('function', 'tool'):
            # Summarize function results
            tool_name = msg.get('name') or msg.get('tool_call_id') or 'unknown'
            result_content = msg.get('content', '')
            condensed.append(f"Tool {tool_name}: {shorten_text(result_content, 500)}")
    
    return "\n".join(condensed)

def build_agent_input_messages(messages):
    """Agent frameworks do not reliably replay prior tool-call history across
    providers. For later turns, pass a compact text history plus the new user
    request, then merge the generated assistant/tool tail back onto the original
    messages.
    """
    if not messages or messages[-1].get("role") != "user":
        return messages
    needs_compact = (
        len([m for m in messages if m.get("role") == "user"]) > 1
        or any(m.get("role") in ("tool", "function") or m.get("tool_calls") or m.get("function_call")
               for m in messages[:-1])
    )
    if not needs_compact:
        return messages
    history = condense_conversation(messages[:-1])
...
def merge_agent_result(original_item, result_item, agent_input_len):
    merged = copy.deepcopy(original_item)
    result_messages = result_item.get("messages") or []
    if result_messages and result_messages[0].get("role") == "system":
        result_messages = result_messages[1:]
    generated_tail = result_messages[agent_input_len:]
    merged["messages"] = merged.get("messages", []) + generated_tail
    if result_item.get("metadata"):
        merged.setdefault("metadata", {}).update(result_item["metadata"])
    return merged

def has_successful_new_assistant(original_item, updated_item):
    new_msgs = (updated_item.get("messages") or [])[len(original_item.get("messages", [])):]
    assistant_msgs = [m for m in new_msgs if m.get("role") == "assistant"]
    if not assistant_msgs:
        return False
    content = assistant_msgs[-1].get("content") or ""
    return not content.startswith("[ERROR") and not content.startswith("[UNEXPECTED_ERROR")
...
def generate_batch_user_queries(items, turn_number, args, input_file_path):
    ...
    user_engine = args.user_simulation_engine or args.engine
    completion_cmd = [
    ...
    if user_engine == 'openrouter_api':
        completion_cmd.extend([
            '--openrouter_url', args.user_simulation_openrouter_url or args.openrouter_url,
            '--openrouter_api_key', args.user_simulation_openrouter_api_key or args.openrouter_api_key
        ])
    elif user_engine == 'vllm_api':
...
def generate_batch_agent_responses(items_with_queries, turn_number, args, input_file_path):
    ...
    for i, (item, query) in enumerate(items_with_queries):
        if query is not None:
            ...
            agent_item = copy.deepcopy(item_with_query)
            agent_item['messages'] = build_agent_input_messages(item_with_query['messages'])
            original_items_by_index[i] = item_with_query
            agent_input_lens_by_index[i] = len(agent_item['messages'])
            batch_items.append(agent_item)
        ...
        if os.path.exists(temp_result_file):
            ...
            for i, (item, query) in enumerate(items_with_queries):
                if i in valid_indices and result_idx < len(batch_results):
                    updated_items.append(merge_agent_result(
                        original_items_by_index[i],
                        batch_results[result_idx],
                        agent_input_lens_by_index[i]))
                    result_idx += 1
...
def generate_multi_turn_conversations_batch(dataset, num_desired_turns, args, input_file_path):
    ...
    # Start from completed conversations only. Current Toucan/datagen rollouts
    # carry ok/finish fields; older prepared datasets do not, so missing fields
    # are treated as usable.
    skipped = {}
    seed_dataset = []
    for item in dataset:
        usable, reason = is_complete_seed(item)
        if usable:
            seed_dataset.append(item)
        else:
            skipped[reason] = skipped.get(reason, 0) + 1
    if skipped:
        print(f"⚠️ Skipped incomplete seed conversations: {skipped}")
    current_dataset = copy.deepcopy(seed_dataset)
    ...
    for turn in range(1, num_desired_turns + 1):
        ...
        for idx, (updated_item, original_item) in zip(active_indices, zip(updated_items, active_items)):
            ...
            if updated_msg_count > original_msg_count and has_successful_new_assistant(original_item, updated_item):
