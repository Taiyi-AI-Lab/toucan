...
prompts=[json.loads(l) for l in open("/data/scripts/Toucan/datagen/prompts_10k_prepared.jsonl")]
def gen(r):
    m=r["metadata"]
    try:
...
with open("/data/scripts/Toucan/datagen/questions_10k.jsonl","w") as f, \
...
print(f"GEN10K_DONE 成功 {ok}/{len(prompts)}",flush=True)
