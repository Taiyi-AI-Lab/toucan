...
from agents import Agent, OpenAIResponsesModel, OpenAIChatCompletionsModel, Runner, SQLiteSession
...
def get_args():
    ...
    parser.add_argument("--model_impl", type=str, default="responses", choices=["responses","chat"], help="chat for deepseek/dimcode (Chat Completions)")
    ...
    parser.add_argument("--parallel_function_calls", type=str2bool, default=True, help="Parallel function calls (default: True)")
...
def normalize_target_tools(target_tools):
    if isinstance(target_tools, str):
        raw_tools = [tool.strip() for tool in target_tools.split(',') if tool.strip()]
    elif isinstance(target_tools, list):
        raw_tools = [str(tool).strip() for tool in target_tools if str(tool).strip()]
    else:
        raw_tools = []
    return [tool.split('::', 1)[1] if '::' in tool else tool for tool in raw_tools]

def synthesize_mcp_servers_from_item(item):
    server = item.get("server") or item.get("metadata", {}).get("server")
    if not server:
        return []
    return [{
        "server_name": server,
        "server_info": {
            "python_sdk_url": f"https://server.smithery.ai/{server}/mcp?config={{config_b64}}&api_key={{smithery_api_key}}&profile={{smithery_profile}}",
...
def create_agent_for_item(item, api_key=None, profile=None):
    ...
    if not mcp_servers or not isinstance(mcp_servers, list):
        mcp_servers = synthesize_mcp_servers_from_item(item)
        if mcp_servers:
            print(f"🔧 Synthesized MCP metadata from server field: {item.get('server')}")
        else:
            return None
    
    # Setup OpenAI model config
    if args.engine == "openrouter_api":
        llm_api_key = args.openrouter_api_key or os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY")
        model = (OpenAIChatCompletionsModel if getattr(args,'model_impl','responses')=='chat' else OpenAIResponsesModel)(
            args.model_path,
...
async def process_single_item_agent_async(item, api_key=None, profile=None):
    ...
    if args.enable_tool_hint:
        ...
        tool_list = normalize_target_tools(target_tools)
