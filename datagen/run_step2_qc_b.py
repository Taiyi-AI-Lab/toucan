...
qs=[json.loads(l) for l in open(f"{HERE}/questions_10k_b_dedup.jsonl")]
...
with open(f"{HERE}/questions_10k_b_qc.jsonl","w") as f, concurrent.futures.ThreadPoolExecutor(max_workers=CONC) as ex:
    for r in ex.map(qc,qs):
        done+=1; ok+=1 if r.get("qc_ok") else 0
        f.write(json.dumps(r,ensure_ascii=False)+"\n"); f.flush()
        if done%500==0: print(f"  {done}/{len(qs)} (ok={ok})",flush=True)
print(f"QCB_DONE 打分成功 {ok}/{len(qs)}",flush=True)
